...
### Frontend タスク（8タスク）
- [ ] 16.1: ThemeService 作成
- [ ] 16.2: ダーク/ライトテーマ定義（Angular Material）
- [ ] 16.3: テーマ切り替え処理実装
- [ ] 16.4: LocalStorage に設定保存
- [ ] 16.5: ThemeToggle コンポーネント作成
- [ ] 16.6: ナビゲーションにテーマ切り替えボタン追加
- [ ] 16.7: カスタム CSS 変数でダークモード対応
- [ ] 16.8: Frontend テスト
...
- [ ] 17.11: エクスポートボタン実装（JSON/CSV 選択）
- [ ] 17.12: ファイルダウンロード処理実装
- [ ] 17.13: ImportDialog コンポーネント作成
- [ ] 17.14: ファイル選択 UI 実装
- [ ] 17.15: インポート処理実装
- [ ] 17.16: インポート結果表示（成功/失敗件数）
- [ ] 17.17: スタイリング
- [ ] 17.18: Frontend テスト
...
### Frontend タスク（10タスク）
- [ ] 18.1: SpeechRecognitionService 作成
- [ ] 18.2: Web Speech API ラッパー実装
- [ ] 18.3: ブラウザ対応チェック実装
- [ ] 18.4: VoiceInput コンポーネント作成
- [ ] 18.5: マイクボタン実装
- [ ] 18.6: 音声認識開始/停止処理
- [ ] 18.7: 認識結果をテキストに変換
- [ ] 18.8: TodoForm に音声入力ボタン追加
- [ ] 18.9: スタイリング（録音中アニメーション）
- [ ] 18.10: Frontend テスト
...
### Frontend タスク（12タスク）
- [ ] 19.1: KeyboardShortcutService 作成
- [ ] 19.2: ショートカット定義（Ctrl+N: 新規作成、Ctrl+F: 検索など）
- [ ] 19.3: キーイベントリスナー実装
- [ ] 19.4: ショートカット実行処理実装
- [ ] 19.5: ショートカット無効化処理（入力フォーカス時）
- [ ] 19.6: ShortcutHelp コンポーネント作成
- [ ] 19.7: ショートカット一覧表示
- [ ] 19.8: ヘルプダイアログ実装（? キーで表示）
- [ ] 19.9: カスタマイズ機能（オプション）
- [ ] 19.10: スタイリング
- [ ] 19.11: Frontend テスト
